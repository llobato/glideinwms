...
class TestLogSupport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # use testfixtures to fool time.time so we don't have to sleep so much waiting for
        # for time to pass so the log files have the right names
        self.time = test_time(
            2018,
            6,
            13,
            ...
            delta=1,
            delta_type='minutes')
        self.replace('glideinwms.lib.logSupport.time.time', self.time)
    ...
    def test_logSupport_time_rotate(self):
        ...
        # turn off testfixtures time stuff here, this test doesn't play
        # nice with it
        self.replace.restore()
        ...
        #print 'required_number_log_attempts=%s'%required_number_log_attempts
        log_attempts = 0
        while log_attempts < required_number_log_attempts:
            #print 'log_attempts %s time %s' % (log_attempts, time.time())
            log.info(create_random_string(length=100))
            log_attempts += 1
